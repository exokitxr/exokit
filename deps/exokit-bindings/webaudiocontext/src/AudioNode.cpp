...
NAN_METHOD(AudioNode::Connect) {
  ...
  if (info[0]->IsObject()) {
    ...
    if (
      ...
      unsigned int outputIndex = info[1]->IsNumber() ? JS_UINT32(info[1]) : 0;
      unsigned int inputIndex = info[2]->IsNumber() ? JS_UINT32(info[2]) : 0;
