...
Local<Value> AudioContext::_DecodeAudioDataSync(Local<Function> audioBufferConstructor, Local<ArrayBuffer> srcArrayBuffer) {
  ...
  if (audioBus) {
    ...
    Local<Value> argv[] = {
      INT32_TO_JS((uint32_t)numChannels),
      INT32_TO_JS((uint32_t)numFrames),
      INT32_TO_JS((uint32_t)audioContext->sampleRate()),
      sourcesArray,
    };
...
Local<Object> AudioContext::CreateBuffer(Local<Function> audioBufferConstructor, uint32_t numOfChannels, uint32_t length, uint32_t sampleRate) {
  Local<Value> argv[] = {
    INT32_TO_JS(numOfChannels),
    INT32_TO_JS(length),
    INT32_TO_JS(sampleRate),
  };
...
Local<Object> AudioContext::CreateScriptProcessor(Local<Function> scriptProcessorNodeConstructor, uint32_t bufferSize, uint32_t numberOfInputChannels, uint32_t numberOfOutputChannels, Local<Object> audioContextObj) {
  Local<Value> argv[] = {
    INT32_TO_JS(bufferSize),
    INT32_TO_JS(numberOfInputChannels),
    INT32_TO_JS(numberOfOutputChannels),
    audioContextObj,
  };
...
NAN_METHOD(AudioContext::New) {
  ...
  float sampleRate = sampleRateValue->IsNumber() ? JS_NUM(sampleRateValue) : lab::DefaultSampleRate;
...
NAN_METHOD(AudioContext::CreateScriptProcessor) {
  ...
  uint32_t bufferSize = info[0]->IsNumber() ? JS_UINT32(info[0]) : 256;
  uint32_t numberOfInputChannels = info[1]->IsNumber() ? JS_UINT32(info[1]) : 2;
  uint32_t numberOfOutputChannels = info[2]->IsNumber() ? JS_UINT32(info[2]) : 2;
...
NAN_GETTER(AudioContext::CurrentTimeGetter) {
  ...
  AudioContext *audioContext = ObjectWrap::Unwrap<AudioContext>(info.This());
  info.GetReturnValue().Set(DOUBLE_TO_JS(audioContext->audioContext->currentTime()));
}
...
NAN_GETTER(AudioContext::SampleRateGetter) {
  ...
  AudioContext *audioContext = ObjectWrap::Unwrap<AudioContext>(info.This());
  info.GetReturnValue().Set(DOUBLE_TO_JS(audioContext->audioContext->sampleRate()));
}
